...
def _get_train_data_loader(batch_size, training_dir, is_distributed):
    ...
    dataset = pd.read_csv(os.path.join(training_dir, "train.tsv"),delimiter="\t")
    sentences = dataset.text_a.values
    labels = dataset.label.values

    input_ids = []
    for sent in sentences:
        encoded_sent = tokenizer.encode(sent, add_special_tokens=True,max_length=MAX_LEN)
...
def _get_test_data_loader(test_batch_size, training_dir):
    dataset = pd.read_csv(os.path.join(training_dir, "test.tsv"),delimiter="\t")
    sentences = dataset.text_a.values
    labels = dataset.label.values
